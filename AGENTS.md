...
- Writing is its own section after Reading. 4th-grade pieces use compact `.writing-grid` cards (same as project cards, smaller) with hero thumbs from `hero/`. 5th writing: **The A Note to Future Me, Sorting Ceremony** (`writing/5thgrade/sortingceremony.html`, hero `hero/sortingceremony.jpg`, thumb `thumbnails/hero/sortingceremony.jpg`) and **Olden** (`writing/5thgrade/olden.html`, hero `hero/olden.jpg`, thumb `thumbnails/hero/olden.jpg`).
